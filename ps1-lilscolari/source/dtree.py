...
class Tree(object):
    ...
    def _choose_feature(self, X, y):
        ...
        Returns
        --------------------
            best_feature   -- int, feature index to split on
    ...
    def _build_helper(self, X, y, node=0, features=None):
        """
        ...
        --------------------
            ...
            y        -- numpy array of shape (n,), target classes
            node     -- int, current node index
            features -- numpy array of shape (d,), feature map
                        features[j] holds index to original feature
        """

        n, d = X.shape

        if features is None:
            features = np.array(range(d))
        ...
        # 2) no features are left
        # 3) all feature values are equal
        if True: # you should modify this condition
            ...
            # make new decision tree node (make sure to use original feature index)

            # split data on best feature

            # build data set for left subtree (x <= threshold)
            # if all values for the chosen feature are the same, remove from data set
            # hint: for a numpy array 'arr' and index 'obj'
            #   to remove a column, use np.delete(arr, obj, axis=1)
            #   to remove a single element, use np.delete(arr, obj)

            # build left subtree using recursion

            # build data set for right subtree (x > threshold)
            # if all values for the chosen feature are the same, remove from data set
    ...
    def fit(self, X, y):
        ...
        ### ========== TODO: START ========== ###
        # part e: fit classifier

        # store classes

        # build tree

        # update node count and resize arrays
